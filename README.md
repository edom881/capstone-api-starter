...
- Adding new products
- Updating product details such as price, description, image, featured status, and stock
- Deleting products
- Creating, updating, and deleting categories

### Current Scope

This project focuses on the backend API. The API can be tested with Swagger UI, Insomnia, Postman, or another HTTP client. It does not require a frontend to test the main workflows.
...
| --- | --- |
| Shopper | Browse products, view categories, manage cart, update profile, checkout |
...
The service gets the logged-in user's cart, creates an order from the user's profile address, creates one order line item for each cart item, then clears the cart after checkout.
...
This code is important because checkout needs to save several related records together. The `@Transactional` annotation helps keep the order creation, line item creation, and cart clearing together as one database operation.
